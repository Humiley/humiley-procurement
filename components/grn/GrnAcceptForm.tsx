...
export type GrnQcLine = { grnLineId: string; description: string; received: string };
...
export function GrnAcceptForm({ grnId, grnNumber, lines }: { grnId: string; grnNumber: string; lines: GrnQcLine[] }) {
  ...
  const [qc, setQc] = useState<Record<string, { a: string; r: string; reason: string }>>(
    Object.fromEntries(lines.map((l) => [l.grnLineId, { a: l.received, r: "0", reason: "" }])),
  );
