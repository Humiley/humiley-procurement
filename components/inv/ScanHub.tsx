  ...
  return (
    <div className="mx-auto max-w-2xl space-y-4">
      ...
      >
        ...
        {cameraSupported ? (
          <button type="button" onClick={() => setCamera(!camera)} className="btn-outline" aria-label={t("camera")}>
      ...
      {camera ? (
        <video ref={videoRef} className="w-full rounded-xl border border-line" muted playsInline />
      ) : null}
      ...
      {result?.kind === "lot" ? (
        <div className="space-y-3 card p-4">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Link href={`/trace/${result.lotId}`} className="btn-emerald btn-sm">
              {t("trace")} →
            </Link>
          </div>
