  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      ...
      {canManage && ["SENT", "PARTIALLY_RECEIVED", "RECEIVED"].includes(status) ? (
        <button className={`${btn} border border-grey/30 text-grey hover:bg-grey/10`} disabled={!!busy} onClick={() => run("close", () => closePo(id))}>
          {busy === "close" ? "…" : t("close")}
        </button>
      ) : null}
      {canManage && ["DRAFT", "APPROVED", "SENT"].includes(status) ? (
        <button className={`${btn} border border-danger/30 text-danger hover:bg-danger/5`} disabled={!!busy} onClick={() => run("cancel", () => cancelPo(id))}>
