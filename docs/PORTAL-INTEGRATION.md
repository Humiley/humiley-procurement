...
## 3. Portal launcher entry

The Humiley Portal has an app launcher (HR / Finance / CRM / Projects). Add:

- **Tile:** "Procurement · Mua hàng" → `https://procurement.humiley.com` (new tab or same tab).
- **Opt-in access** like the other portal apps: the portal's `appsDenied`/opt-in model controls
  who SEES the tile; actually being able to sign in is governed by having a procurement `User`
  row (step 2.3). Both lists are admin-curated; the email is the shared key.
- Optional deep links from portal widgets: `/approvals` (pending count), `/payment-requests`,
  `/reports` — all routes are stable and role-guarded server-side.
...
4. Add the launcher tile in the portal config.
