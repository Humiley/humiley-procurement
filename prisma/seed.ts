...
// ---- Phase 14: §20 trade reference (C/O forms, HS codes, duty matrix, item links, FX) ----
async function seedTradeReference() {
  ...
  const ITEM_TRADE: [string, string, string, boolean][] = [
    ["HVAC-AHU-05", "8415.83", "CN", false],
    ["HVAC-FCU-02", "8415.83", "CN", false],
    ["HVAC-HEPA-14", "8421.39", "CN", false],
    ["HVAC-DMPR-30", "8481.80", "CN", false],
    ["HVAC-VFD-15", "8504.40", "CN", true],
    ["ELEC-MCB-32", "8536.20", "KR", false],
    ["ELEC-CONT-40", "8536.49", "KR", false],
    ["ELEC-CBL-25", "8544.49", "VN", false],
  ];
  for (const [itemCode, hsCode, origin, license] of ITEM_TRADE) {
    const item = await db.item.findUnique({ where: { code: itemCode } });
    if (!item) continue;
    await db.itemTrade.upsert({
      where: { itemId: item.id },
      update: { hsCodeId: hsIds.get(hsCode)!, originCountry: origin, requiresImportLicense: license, licenseNote: license ? "Energy-efficiency test report (MOIT) before customs release." : null },
      create: { itemId: item.id, hsCodeId: hsIds.get(hsCode)!, originCountry: origin, requiresImportLicense: license, licenseNote: license ? "Energy-efficiency test report (MOIT) before customs release." : null },
    });
  }
  ...
  console.log(`Seeded §20 trade reference: ${FORMS.length} C/O forms, ${HS.length} HS codes + duty matrix, ${ITEM_TRADE.length} item links, ${FX.length} FX rates.`);
}
